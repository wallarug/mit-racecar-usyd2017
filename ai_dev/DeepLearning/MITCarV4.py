...
def load_data_from_directory(dir_path, image_size = None, use_throttle = False, use_steering = False):
    ...
    data = np.zeros((num, image_size[0], image_size[1], 4), dtype="float32")
...
def train(data, label, input_shape, nb_class):
    ...
    if(data is not None and label is not None):
        model.fit(data, label, batch_size=64, epochs=1, validation_split=0.2, verbose=1, shuffle=True, callbacks=callbacks_list)
...
if __name__ == "__main__":
    run("./usb_capture/dataset/",
        "./usb_capture/dataset/", use_throttle=False,
        use_steering=True, image_size=(250, 250, 4))