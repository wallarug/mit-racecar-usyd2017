    ...
    while i < num_images:
        ...
        if left_bumper:
            ...
            if zed.grab(runtime_parameters) == tp.PyERROR_CODE.PySUCCESS:
                ...
                t = time.time()
                print('writing dataset_l/image_{0:.2f}_{1:.4f}_{2:.4f}.pickle'.format(t,throttle,steering))
                if (i%25==0):
                    print('{} images have been captured'.format(i))

                pickle.dump(merged,open( 'dataset_l/image_{0:.2f}_{1:.4f}_{2:.4f}.pickle'.format(t,throttle,steering), 'wb' ))
            else:
