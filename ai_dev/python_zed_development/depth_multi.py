...
def multi_threaded_merge(data,depth):
    from multiprocessing import Pool
    n_threads=3
    p=Pool(n_threads)
    # group rows together
    all_rows = zip(data,depth)
    # send rows to be merged
    output=p.map(thread_merge_row,all_rows)
    return output

def thread_merge_row(args):
    data,depth=args
    # move across the row
    for col in range(len(data)):
        # change each pixel in the row
        data[col][3]=depth[col][0] #keep only the first value out of 4 depth values
    return data
...
def load_and_merge(num):
    ...
    start_time=time.time()
    merged = multi_threaded_merge(image_rgb,image_depth)
    elapsed_time=time.time()-start_time
